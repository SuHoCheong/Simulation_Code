...
def Particle(t, tau,camera):
    ...
    x0array = np.linspace(2,98,15)
    #x0array = [0.4,0.6]
    y0array = np.linspace(2,98,15)
    #y0array = [0.4,0.6]

    FR = 0.3 #Flocking Radius
    N = 225
    particle_array = []
    xplot = []
    yplot = []
            ...
            for j in range(N):
                if u != j:
                    ...
                    if inside_radius(xu,yu,xj,yj,R,mod):
                        ...
                        particle_array[u].setNEWtheta((particle_array[u].gettheta() + particle_array[j].gettheta())/2)
                        particle_array[j].setNEWtheta((particle_array[u].gettheta() + particle_array[j].gettheta())/2)
